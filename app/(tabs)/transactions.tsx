...
export default function TransactionsScreen() {
  ...
  const load = useCallback(async () => {
    if (filter === 'customer' && selectedCustomerId) {
      const all = await getTransactionsByDateRange(db, '2000-01-01', '2099-12-31');
      return setTransactions(all.filter(t => String(t.customer_id) === selectedCustomerId));
    }
    const dateStr = selectedDate.toISOString().slice(0, 10);
    setTransactions(await getTransactionsByDateRange(db, dateStr, dateStr));
  ...
  const handleCustomerSelect = (custId: string) => {
    ...
    getTransactionsByDateRange(db, '2000-01-01', '2099-12-31').then(all =>
      setTransactions(all.filter(t => String(t.customer_id) === custId))
    );
  };
        ...
        <View style={S.cardContent}>
          ...
          <Text style={S.cardSub} numberOfLines={1}>
            ...
            {item.description && item.description !== 'Kuboos' && item.description !== 'Payment received'
              ? ` · ${item.description}` : ''}
            {filter === 'customer' ? ` · ${format(new Date(item.date), 'dd MMM')}` : ''}
          </Text>
        </View>
      </TouchableOpacity>
    );
